...
/**
 * TODO: java.sql.ResultSet 的 包装器，可以理解成 ResultSet 的工具类，提供给 DefaultResultSetHandler 使用
 *
 * @author Iwao AVE!
 */
public class ResultSetWrapper {
   ...
   * 获取读取结果集时要使用的类型处理程序。
   * 尝试通过搜索属性类型从TypeHandlerRegistry获取。
   * 如果没有找到它，则获取列JDBC类型并尝试获取它的处理程序。
   *
   * TODO: 获得指定字段名的指定 JavaType 类型的 TypeHandler 对象
   *
   * @param propertyType
   * @param columnName
   * @return
  ...
  public TypeHandler<?> getTypeHandler(Class<?> propertyType, String columnName) {
    ...
    if (handler == null) {
      ...
      // <2> 基于 propertyType + jdbcType ，获得 TypeHandler 对象
  ...
  private void loadMappedAndUnmappedColumnNames(ResultMap resultMap, String columnPrefix) throws SQLException {
    ...
    // <3> 将 mappedColumnNames 和 unmappedColumnNames 结果，添加到 mappedColumnNamesMap 和 unMappedColumnNamesMap
    // #getMapKey(ResultMap resultMap, String columnPrefix) 方法，获得缓存的 KEY 。
