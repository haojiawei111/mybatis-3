...
public abstract class BaseExecutor implements Executor {
  ...
  // 调用 #doUpdate(MappedStatement ms, Object parameter) 方法，执行写操作。这是个抽象方法，由子类实现
  protected abstract int doUpdate(MappedStatement ms, Object parameter)
      throws SQLException;

  // 调用 #doFlushStatements(boolean isRollback) 方法，执行刷入批处理语句。这是个抽象方法，由子类实现
  protected abstract List<BatchResult> doFlushStatements(boolean isRollback)
      throws SQLException;

  // 调用 #doQuery(MappedStatement ms, Object parameter, RowBounds rowBounds, ResultHandler resultHandler, BoundSql boundSql) 执行读操作。这是个抽象方法，由子类实现
  ...
  protected void applyTransactionTimeout(Statement statement) throws SQLException {
    StatementUtil
        .applyTransactionTimeout(statement, statement.getQueryTimeout(), transaction.getTimeout());
  }
